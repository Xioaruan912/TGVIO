...
# This only replaces tgvio-player with an explicitly supplied prior image.
repo_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
compose_file="$repo_root/docker-compose.player.yml"

usage() {
  printf 'usage: %s --env-file PLAYER_ENV --image PRIOR_PLAYER_IMAGE --execute\n' "$0" >&2
  exit 2
}
...
[[ "$execute" == true && -n "$env_file" && -n "$image" ]] || usage
...
[[ "$image" != *$'\n'* ]] || { printf 'invalid Player image reference\n' >&2; exit 2; }
...
bot_before=$(docker inspect --format '{{.Id}} {{.State.Status}}' tgvio 2>/dev/null) || {
  printf 'refusing Player rollback: Bot container tgvio is not inspectable\n' >&2
  exit 2
}
[[ "$bot_before" == *' running' ]] || { printf 'refusing Player rollback: Bot is not running\n' >&2; exit 2; }

temporary=$(mktemp)
trap 'rm -f "$temporary"' EXIT
sed "s|^TGVIO_PLAYER_IMAGE=.*$|TGVIO_PLAYER_IMAGE=$image|" "$env_file" >"$temporary"
chmod 600 "$temporary"
docker compose --project-name tgvio-player --env-file "$temporary" -f "$compose_file" --profile player \
...
bot_after=$(docker inspect --format '{{.Id}} {{.State.Status}}' tgvio 2>/dev/null) || {
  printf 'Player rollback changed Bot container visibility\n' >&2
  exit 1
}
[[ "$bot_after" == "$bot_before" ]] || {
  printf 'Player rollback changed the Bot container; investigate before further actions\n' >&2
  exit 1
}
printf 'player_rollback=started bot_container_unchanged=true\n'
