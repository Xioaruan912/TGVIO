...
| UI-06 | 发布前可编辑合集（封面/排序/移除/文案），多草稿可保存/列表/继续/重启恢复；确认幂等且 revision 失效旧预览 | VERIFIED | R2-16（`8cd0ac1` + `0012_collection_editing`）交付 `collection_drafts`/`collection_entry_edits`/`collection_submissions`/`editing_interactions`；每 owner/chat 单个活动草稿，确认使用 owner/revision/TTL/single-use operation token + snapshot hash，重复确认只产生一次副作用。422 tests、v12 postflight |
| UI-07 | 结果卡分别展示 Telegram 与 Archive 结果，可打开帖子、再发、同款再发、收藏、分享、撤销；收藏在历史隐藏后仍可访问 | VERIFIED | R2-18B（`bd7ae31` + `0013_owner_favorites`）：`favorites` + `user_preferences.quiet_mode/style_json`；打开帖子仅在公开 `@username` 且有已确认频道 effect 时生成，否则解释原因；分享只发链接到当前私聊；收藏 SQL 分页且不随每日隐藏删除。433 tests、v13 postflight |
...
| UI-10 | owner 主动生成可视化效果预览：有界下载/缓存/耗时预算，不创建发布 Job、不触发 Archive、保护 spoiler、绑定 revision | VERIFIED | R2-18D（`1a9b86c`，migration=none）：`preview_requests`；最多 1 份封面来源、32 MiB/30 s/并发 1·2，独立缓存目录用后即删，启动清理未完成请求。446 tests、postflight |
