...
| DB-02 | schema 使用不可变、有 checksum 的前向 migration，并在启动前备份和校验 | VERIFIED | R2-03B 接管 checksum ledger；R2-04 `0002_scheduler` 已正式把生产推进到 `user_version=2`，production-shape rehearsal、SQLite backup API、schema fingerprint/checksum fail-closed 与 rollback asset check 均通过 |
