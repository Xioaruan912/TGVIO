...
| UI-06 | 发布前可编辑合集（封面/排序/移除/文案），多草稿可保存/列表/继续/重启恢复；确认幂等且 revision 失效旧预览 | VERIFIED | R2-16（`8cd0ac1` + `0012_collection_editing`）交付 `collection_drafts`/`collection_entry_edits`/`collection_submissions`/`editing_interactions`；每 owner/chat 单个活动草稿，确认使用 owner/revision/TTL/single-use operation token + snapshot hash。R2-18F2（`568acc6` + `0015_frozen_submissions`）把授权校验+CAS 消费 token+完整 `frozen_json`+submission 放到 T1 单事务，并按 `collection_part_jobs(session_id, part_index)` 幂等创建分块，支持崩溃恢复且不同 token fail closed。472 tests、v16 postflight |
| UI-07 | 结果卡分别展示 Telegram 与 Archive 结果，可打开帖子、再发、同款再发、收藏、分享、撤销；收藏在历史隐藏后仍可访问 | VERIFIED | R2-18B（`bd7ae31` + `0013_owner_favorites`）：`favorites` + `user_preferences.quiet_mode/style_json`；打开帖子仅在公开 `@username` 且有已确认频道 effect 时生成，否则解释原因；分享只发链接到当前私聊；收藏 SQL 分页且不随每日隐藏删除。R2-18F3（`07bc973` + `0016_draft_style`）draft-scoped 风格与同款再发不污染 owner 默认；R2-18F5（`b238b58`）收藏改为幂等目标状态、撤销 effect 过滤与链接去重、分享失败不报成功。472 tests、v16 postflight |
| UI-08 | 发布风格（极简直发/封面合集/图文精选/我的常用）说明、单次调整与恢复默认；确认时冻结到任务，不静默关闭 spoiler | VERIFIED | R2-18C（`01f9dbc`，migration=none）：风格写入 `job.policy["publish_style"]`，`JobOrchestrator` 优先读 job 快照、缺失回退全局；常驻键盘切到最终六键。436 tests、postflight |
| UI-09 | 本地规则整理建议（封面/排序/重复/疑似重复），主动应用、可撤回、绑定 revision，不自动改动内容 | VERIFIED | R2-18E（`5a917b0` + `0014_suggestion_and_preview`）：`suggestion_applications` durable undo；有 SHA 证据才称精确重复，否则“疑似”。441 tests、v14 postflight |
| UI-10 | owner 主动生成可视化效果预览：有界下载/缓存/耗时预算，不创建发布 Job、不触发 Archive、保护 spoiler、绑定 revision | VERIFIED | R2-18D（`1a9b86c`，migration=none）：`preview_requests`；最多 1 份封面来源、32 MiB/30 s/并发 1·2，独立缓存目录用后即删。R2-18F4（`4acd2bc`）增加实际写入硬字节预算（超限取消下载）、有界队列+单 owner admission、总 deadline 覆盖排队/下载/生成/发送、启动前/发送前重查 TTL/revision/状态、分页路径安全清理与 runtime 生命周期回收。472 tests、postflight |
