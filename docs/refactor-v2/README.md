...
> 状态：R2-00～R2-03 已交付；R2-04 A-E 代码均已正式发布 HostDZire，R2-04E bounded concurrent upload 仍待 1～3 个真实大文件 performance acceptance 后关闭阶段（2026-09-12）
...
本次不是在旧 `src/bot.py` 上继续拆 facade，也不是再写第三套实现。HostDZire 正在运行 clean-room rewrite（包名 `tgvio`），当前生产 R2-04 A-E 已具备 durable Job、PublishPlan、side-effect journal、Telegram 发布、Archive V2、恢复、诊断、singleton runtime lease、generation-fenced phase claim、strict FIFO ordered publish dispatcher 与 bounded concurrent Telegram upload；最新正式 release gate 为 214 tests。V2 重构以这套生产源码为唯一代码基线，按可回滚阶段继续治理。
...
- 当前生产 release 为 `r2-04-05d4bf0-20260912T072358Z`，runtime commit 为 `05d4bf02f8dbc42eb9ace2fd572270b3a5d84750`；R2-04 A-D 交付证据见 [R2-04_RELEASE.md](evidence/R2-04_RELEASE.md)，R2-04E 代码发布证据见 [R2-04E_RELEASE.md](evidence/R2-04E_RELEASE.md)。
...
- R2-04 A-D 已通过 `0002_scheduler` 正式推进生产到 `user_version=2`，singleton runtime lease、generation-fenced prepare/publish/archive claim、durable `accepted_order` 与单 ordered publish dispatcher 已上线；100 Job 随机 readiness 仍严格按 accepted order 发布，`publish_partial/uncertain` 会阻塞后续自动发布。R2-04E 已以 `migration=none` 发布 `r2-04-05d4bf0-20260912T072358Z`，恢复 legacy 16 路 Telegram MTProto 分片上传，并增加全局并发上限、失败前置 fallback、吞吐日志与可见消息 exactly-once 回归；正式 release gate 为 214 tests / 21.362s。独立 postflight 证明 schema 仍为 v2 同一 hash、23 Job 不变、runtime lease active=1、container healthy/restart=0，rollback check 通过。真实 1～3 个大文件吞吐/CPU/内存/FloodWait 仍需生产观测，所以 R2-04 总阶段和 PL-10 暂不标最终 VERIFIED。A-D 证据见 [R2-04_RELEASE.md](evidence/R2-04_RELEASE.md)，E release 证据见 [R2-04E_RELEASE.md](evidence/R2-04E_RELEASE.md)。
