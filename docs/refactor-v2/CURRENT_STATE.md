...
> 阶段说明：R2-03A/B 已交付；R2-04 A-D durable scheduler 与 R2-04E bounded concurrent upload 均已正式发布 HostDZire 并完成独立 postflight/rollback-check。当前生产事实以 `r2-04-05d4bf0-20260912T072358Z` 为准；R2-04E 只剩 1～3 个真实大文件 performance acceptance 尚未关闭。

## 1. 源码权威已经对齐

| 范围 | 当前事实 | 结论 |
|---|---|---|
| Git runtime 基线 | `05d4bf02f8dbc42eb9ace2fd572270b3a5d84750` 已推送 `origin/main` | 这是当前生产 TGVIO runtime 的完整 Git object |
...
| 生产 release | `r2-04-05d4bf0-20260912T072358Z`；`.release-commit` 与容器 `APP_COMMIT` 均为 full `05d4bf02f8dbc42eb9ace2fd572270b3a5d84750` | release 身份由 full commit、manifest 与不可变 image 共同固定 |
| 生产 image | `sha256:2aaf8b84b64446409bc0e65fca152a3d8c2b42afadee24644a821de2ec924353` | 正式 runtime image；部署前 image 仍由 rollback tag 保留 |
| Runtime 源码 | Git、生产宿主、运行容器的规范化 Python manifest 均为 `2971c3da4c737b0c938a2f6ba31ccfcdc883fd0c32bea58a8c2aaf94bdfa4773` | R2-04 A-E 业务源码三方身份一致 |
...
- Source：`/root/TGVIO-releases/r2-04-05d4bf0-20260912T072358Z/source`，由 `/root/TGVIO-current` 原子指向。
- 容器 ID：`4ccaf2612a764ec55afbd73b08b6a2f5d18fd4e368f1231ea380cc6b22a63995`。
...
- Scheduler migration/cutover 证据见 [R2-04_RELEASE.md](evidence/R2-04_RELEASE.md)；bounded upload release/postflight 证据见 [R2-04E_RELEASE.md](evidence/R2-04E_RELEASE.md)。
...
- R2-04 A-D schema-changing cutover 前已使用 SQLite backup API 创建 pre-migration backup；R2-04E 使用 `migration=none` 且独立 postflight 证明 schema hash 完全不变；`rollback_hostdzire.sh --check r2-04-05d4bf0-20260912T072358Z` 通过。

### 2.3 代码、测试与镜像

- 当前生产 R2-04 A-E runtime：57 个 Python 源文件；原 `infrastructure/sqlite.py` hotspot 继续保持 facade + 分模块结构，scheduler 边界与 `0002_scheduler.sql` 保持不变，并新增独立 `adapters/telegram/uploads.py`；R2-04E 没有新增 migration。
...
- 当前生产的 domain/application AST 依赖边界与 57-file architecture gate 通过。
- 当前生产 R2-04E 正式 Docker test target 在 `--network none` 下通过 214 tests（发布机 21.362 秒），覆盖既有 scheduler/migration/release 合同，并新增小/大文件 SavePart、legacy 16-way、global cap、cancel/fallback、transport exactly-once visible-send 回归；没有启动第二个 Telegram Bot。
...
- 正式 runtime image `sha256:2aaf8b84b64446409bc0e65fca152a3d8c2b42afadee24644a821de2ec924353` 已通过 release image inspection；生产 source/image/commit identity 与独立 postflight 一致。
...
2. **功能兼容仍有缺口**：strict FIFO/durable claim 和 R2-04E bounded upload 均已生产发布；R2-04E 仍待 1～3 个真实大文件的吞吐/CPU/内存/FloodWait 观测后才能把 PL-10 升为 VERIFIED。合集会话/文字、spoiler 偏好、hold/resume、undo、分页失败中心、动态 Archive、多目的地、代理和 Dashboard 仍为后续合同项。
...
- 禁止把生产 R2-04E 的 214 tests 等同于所有旧功能已恢复；R2-04E 仍需真实大文件性能观测，后续合同继续逐项验收。
