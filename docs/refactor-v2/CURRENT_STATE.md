...
> 阶段说明：R2-03A/B、R2-05、R2-06、R2-07A1/A2/A3/D、R2-08、R2-09、R2-11～R2-15 均已交付，R2-10 自动化收口已完成；R2-16 合集编辑/草稿、R2-18 用户体验升级（18A～E）与 R2-18F 验收缺陷修复（F1～F7）已实现并部署；R2-17 完整内容审核仍未实现。所有 `REQUIRED` 合同为 `VERIFIED`。当前生产事实以 `r2-18f7-31a51d2-20260915T025757Z` / schema v16 为准。R2-04E 真实大文件性能、R2-05/06 真实 E2E 与破坏性回滚演练仍需 owner 窗口，清单见 [R2-10_CLOSURE.md](evidence/R2-10_CLOSURE.md)。

## 1. 源码权威已经对齐

| 范围 | 当前事实 | 结论 |
|---|---|---|
| Git runtime 基线 | `31a51d26eadbbb7aacc1bbc584bd04aafc1ed6ca` 已推送 `origin/main` | 当前生产 TGVIO runtime 的完整 Git object；R2-18F 最终包（F7 效果预览入口） |
...
| 生产 release | `r2-18f7-31a51d2-20260915T025757Z`；`.release-commit` 与容器 `APP_COMMIT` 均为 full `31a51d26eadbbb7aacc1bbc584bd04aafc1ed6ca` | release 身份由 full commit、manifest 与不可变 image 共同固定 |
| 生产 image | `sha256:52d96b8c2aea5a263754a7fb559fefd03f54665f0d1dfb335732e5acd991e93f` | 正式 R2-18F7 runtime image；匹配 source manifest、v16 DB 与 rollback point 均可验证 |
| Runtime 源码 | Git、生产宿主、运行容器的 source manifest 均为 `7759913efd2ef44ca30b8d273e4ff1ddbb9a41209c52eab4646dd100b46d2b23` | R2-18F7 runtime 三方身份一致 |
...
- Release：`r2-18f7-31a51d2-20260915T025757Z`。
- Source：`/root/TGVIO-releases/r2-18f7-31a51d2-20260915T025757Z/source`，由 `/root/TGVIO-current` 原子指向。
...
- 当前 `jobs_total=3`（均为终态 succeeded 历史任务；R2-15 起不再物理删除）；非终态 Job blocker 为 0。
...
- `PRAGMA user_version=16`，`schema_migrations` 已连续登记 `0001_baseline`～`0016_draft_style`。
- 规范化整库 schema SQL SHA-256 为 `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c`。
...
- R2-16 以 `0012_collection_editing` 推进到 v12：新增 `collection_drafts`/`collection_entry_edits`/`collection_submissions`/`editing_interactions`，并把既有 open 会话回填为活动草稿；旧的 open-session 唯一索引被活动草稿索引替换。
- R2-18B 以 `0013_owner_favorites` 推进到 v13：新增 `favorites` 与 `user_preferences.quiet_mode/style_json`。
- R2-18E 以 `0014_suggestion_and_preview` 推进到 v14：新增 `suggestion_applications`（建议撤回）与 `preview_requests`（效果预览请求）。R2-18C/R2-18D 为 migration=none。
- R2-18F2 以 `0015_frozen_submissions` 推进到 v15：`collection_submissions.frozen_json`/`token_id` 与 `collection_part_jobs`（分块幂等身份）。
- R2-18F3 以 `0016_draft_style` 推进到 v16：`collection_drafts.style_json`（draft-scoped 风格）。R2-18F1/F4/F5/F6/F7 为 migration=none。

### 2.3 代码、测试与镜像

- 当前生产 R2-18F7 runtime：123 个 Python 源文件；schema 为 v16。R2-18F 新增持久化冻结提交/分块幂等、draft-scoped 风格、预览字节预算与生命周期、收藏幂等、启动提交恢复与效果预览入口。
...
- 当前生产 R2-18F7 正式 clean/pushed Docker test target 在 `--network none` 下通过 479 tests，覆盖既有 scheduler/migration/upload/intake/release/undo/Archive/diagnostics、Dashboard/metrics/outbox、告警/预览、归档容错、短路径布局、安全隐藏/维护、`job_display_identity`、R2-16 草稿 revision/冻结提交、R2-18 结果卡/收藏/分享/安静模式/风格/建议/预览，以及 R2-18F 冻结快照崩溃恢复/分块幂等/双 token 竞争、预览硬字节预算与路径安全、收藏目标状态幂等、F6 启动恢复与冻结后拒收、F7 预览入口路由；没有启动第二个 Telegram Bot。
...
- 正式 runtime image `sha256:52d96b8c2aea5a263754a7fb559fefd03f54665f0d1dfb335732e5acd991e93f` 已通过 release image inspection；生产 source/image/commit identity 与独立 postflight 一致。
...
- 合集编辑与草稿（R2-16）：发布前可选封面、移除/恢复、上移/下移、改文案；多草稿保存/列表/继续/重启恢复；revision/CAS + operation token 冻结提交，重复/并发确认只产生一次副作用。
- 用户体验升级（R2-18）：上下文首页与最终六键键盘；结果卡分别显示 Telegram 与 WebDAV，公开 `@username` 才生成帖子链接；owner-scoped 收藏/分享/安静模式；内置+自定义发布风格并冻结到任务；本地规则整理建议（可撤回）；owner 主动触发的有界效果预览（不创建 Job、不触发 Archive、用完即删缓存、保护 spoiler）。
- 验收缺陷修复（R2-18F）：确认使用 T1 原子「校验+CAS 消费 token+完整冻结快照+submission」并支持崩溃恢复与分块幂等，不同 token fail closed；draft-scoped 风格优先级与同款再发不污染 owner 默认；预览按实际写入执行硬字节预算、有界队列/TTL、分页安全清理与生命周期回收；收藏以目标状态幂等写入，结果卡对撤销 effect 与重复链接去重；F6 冻结后拒收迟到媒体/文字、启动恢复 `creating` 提交、预览流式预算与路径 fail-closed；F7 收集卡“👀 预览与整理”与首层“🖼 生成效果预览”，清理时间动态显示（生产 `daily_cleanup_time=19:00`）。
...
4. **VPS 时间同步**：2026-09-15 最新生产报告为 `ntp_synchronized=yes`，此前 `no` 的问题已消除。
