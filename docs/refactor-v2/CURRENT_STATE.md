...
> 阶段说明：R2-03A/B 已交付；R2-04 A-D durable scheduler 已通过 `0002_scheduler` 正式发布 HostDZire 并完成独立 postflight/rollback-check。当前生产事实仍以 `r2-04-0016988-20260912T070451Z` 为准；R2-04E bounded concurrent upload 已完成 local candidate 与 214 项离线门禁，但尚未发布生产。
...
- 当前生产 R2-04 A-D runtime：56 个 Python 源文件；原 `infrastructure/sqlite.py` hotspot 继续保持 facade + 分模块结构，并新增 domain/application/infrastructure scheduler 边界与 `0002_scheduler.sql`。R2-04E local candidate 为 57 个 Python 源文件，新增独立 `adapters/telegram/uploads.py`，没有新增 migration。
...
- 当前生产的 domain/application AST 依赖边界与 56-file architecture gate 已通过；R2-04E candidate 的 57-file architecture gate 同样通过。
- 当前生产 R2-04 A-D 正式 Docker test target 在 `--network none` 下通过 205 tests（发布机 21.483 秒），新增 100-Job FIFO、跨 SQLite connection lease/claim、claim-loss cancellation、v1→v2 rehearsal、v2 schema drift fail-closed 和 graceful handoff contract。R2-04E candidate 最新同 target 通过 214 tests（23.028 秒），增加小/大文件 SavePart、legacy 16-way、global cap、cancel/fallback、transport exactly-once visible-send 回归；所有测试均未启动第二个 Telegram Bot。
...
2. **功能兼容仍有缺口**：严格 FIFO/durable claim 已在 R2-04 A-D 生产发布；R2-04E 已有恢复 legacy 16 路上传能力的 local candidate，但尚待正式发布及 1～3 个真实大文件的吞吐/CPU/内存/FloodWait 观测。合集会话/文字、spoiler 偏好、hold/resume、undo、分页失败中心、动态 Archive、多目的地、代理和 Dashboard 仍为后续合同项。
...
- 禁止把生产 R2-04 A-D 的 205 tests 或 R2-04E candidate 的 214 tests 等同于所有旧功能已恢复；R2-04E 仍需正式发布和真实大文件性能观测，后续合同继续逐项验收。
