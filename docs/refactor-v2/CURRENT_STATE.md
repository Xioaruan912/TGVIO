...
> 阶段说明：R2-03A/B、R2-05、R2-06、R2-07A1/A2/A3/D、R2-08、R2-09 均已交付，R2-10 自动化收口已完成；所有 `REQUIRED` 合同为 `VERIFIED`。R2-08 分两次发布完成全部热点拆分并把源文件预算收紧到 1000 行，R2-09 以 `0008_notification_outbox` 把生产推进到 schema v8。当前生产事实以 `r2-08b-52f39ca-20260913T101837Z` / schema v8 为准。R2-04E 真实大文件性能、R2-05/06 真实 E2E 与破坏性回滚演练仍需 owner 窗口，清单见 [R2-10_CLOSURE.md](evidence/R2-10_CLOSURE.md)。

## 1. 源码权威已经对齐

| 范围 | 当前事实 | 结论 |
|---|---|---|
| Git runtime 基线 | `52f39cad6c33614daaa742ed2f07dea0c4a6011d` 已推送 `origin/main` | 这是当前生产 TGVIO runtime 的完整 Git object；R2-08B 热点拆分实现位于该提交 |
...
| 生产 release | `r2-08b-52f39ca-20260913T101837Z`；`.release-commit` 与容器 `APP_COMMIT` 均为 full `52f39cad6c33614daaa742ed2f07dea0c4a6011d` | release 身份由 full commit、manifest 与不可变 image 共同固定 |
| 生产 image | `sha256:507d38f97c3d5d3d60d2d2295ec2c5e850d053554e70cc6af73d9ea6bdf3bd0e` | 正式 R2-08B runtime image；匹配 source manifest、v8 DB 与 rollback point 均可验证 |
| Runtime 源码 | Git、生产宿主、运行容器的 source manifest 均为 `9d2b27c3b058c5ee2de70a34e4472eab3c6bdc354fd882bb2c2b3342fe394fb9` | R2-08B 热点拆分 runtime 三方身份一致 |
...
- Source：`/root/TGVIO-releases/r2-08b-52f39ca-20260913T101837Z/source`，由 `/root/TGVIO-current` 原子指向。
- 容器 ID：`71d3f7764a329b2d74d3e2e58b1852c67c7ae3d511a17ecd4cdba44443e246e8`。
- Started-at：`2026-09-13T10:15:37.801509677Z`。
...
- 当前生产 R2-08B runtime：98 个 Python 源文件；schema 仍为 v8（migration=none）。R2-08/R2-08B 为行为等价的热点 mixin 拆分，最大文件 945 行；mixin 覆盖 `bot_ui`、publish transport、WebDAV client、Archive executor、intake runtime 与 SQLite archive deletion。
...
- 当前生产的 domain/application AST 依赖边界与 98-file architecture gate 通过，并强制 `MAX_SOURCE_FILE_LINES=1000` 单文件预算。
- 当前生产 R2-08B 正式 clean/pushed Docker test target 在 `--network none` 下通过 368 tests，覆盖既有 scheduler/migration/upload/intake/release/undo/Archive/diagnostics 合同、R2-09 的 Dashboard/metrics/outbox，以及 R2-08/R2-08B mixin 拆分与 1000 行源文件预算；没有启动第二个 Telegram Bot。
...
- 正式 runtime image `sha256:507d38f97c3d5d3d60d2d2295ec2c5e850d053554e70cc6af73d9ea6bdf3bd0e` 已通过 release image inspection；生产 source/image/commit identity 与独立 postflight 一致。
