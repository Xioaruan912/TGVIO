...
> 阶段说明：R2-03A/B、R2-05、R2-06 已交付；R2-04 A-E 已正式发布但 E 仍待 1～3 个真实大文件 performance acceptance。R2-07A1/A2/A3/D 均已正式发布，R2-07 收口。当前生产事实以 `r2-07d-10b6dd5-20260913T091106Z` / schema v7 为准。

## 1. 源码权威已经对齐

| 范围 | 当前事实 | 结论 |
|---|---|---|
| Git runtime 基线 | `10b6dd5f58f8839a6fa51428a32b93d8f95c098c` 已推送 `origin/main` | 这是当前生产 TGVIO runtime 的完整 Git object；R2-07D 实现位于该提交，未改变 schema |
...
| 生产 release | `r2-07d-10b6dd5-20260913T091106Z`；`.release-commit` 与容器 `APP_COMMIT` 均为 full `10b6dd5f58f8839a6fa51428a32b93d8f95c098c` | release 身份由 full commit、manifest 与不可变 image 共同固定 |
| 生产 image | `sha256:0b88a34b1055015f2397b164c0ee9885c1cf5fe3cf9552c501f54e4c35051333` | 正式 R2-07D runtime image；匹配 source manifest、v7 DB 与 rollback point 均可验证 |
| Runtime 源码 | Git、生产宿主、运行容器的 source manifest 均为 `5d29d0b9d6a685eb8b3e98fbbc1b62be574ac55b01f9412e28f77f8f892c4333` | R2-07D Diagnostic Snapshot runtime 三方身份一致 |
...
- Source：`/root/TGVIO-releases/r2-07d-10b6dd5-20260913T091106Z/source`，由 `/root/TGVIO-current` 原子指向。
...
- R2-07D 以 `migration=none` 发布，不改变 schema：生产仍为 `user_version=7`、ledger `1..7`、schema hash `9cf2d4008d4fb888f5affdffea1ccd413b51968d14d234e07d0769cfe60c6e90`；启动新增一条脱敏 `static_proxy` runtime-health 行（未配置时为 `disabled`）。

### 2.3 代码、测试与镜像

- 当前生产 R2-07D runtime：72 个 Python 源文件；schema 仍为 v7（migration=none），新增固定 `DiagnosticSnapshot` 服务、3 条有界 SQL 聚合与静态代理有界探测。
...
- 当前生产 R2-07D 正式 clean/pushed Docker test target 在 `--network none` 下通过 350 tests，覆盖既有 scheduler/migration/upload/intake/release/undo/Archive 合同，以及固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界探测、离线 `--check` 的 `configured_unchecked` 与 secret/path fixture 拒绝；没有启动第二个 Telegram Bot。
...
2. **功能兼容仍有缺口**：R2-05 已恢复合集会话/文字与 spoiler 偏好但仍需真实交互验收；R2-04E 仍待 1～3 个真实大文件观测；R2-06 真实 destructive undo 仍需受控验收。R2-07 的单一 Archive Profile/策略、精确删除与安全诊断摘要已全部交付；动态多目的地和动态代理已由用户明确退役，静态环境代理启动检测/脱敏状态已由 R2-07D 交付；Dashboard 仍为后续合同项。
