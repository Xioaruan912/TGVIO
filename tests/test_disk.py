...
from src.services.disk import CleanupCandidate, DiskManager
...
class DiskManagerTests(unittest.TestCase):
    ...
    def test_cleanup_candidate_explicitly_deletes_tree_and_rechecks_partial(self) -> None:
        manager = DiskManager(self.root)
        job_dir = self.root / "job-8"
        nested = job_dir / "nested"
        nested.mkdir(parents=True)
        (job_dir / "a.mp4").write_bytes(b"a" * 5)
        (nested / "b.jpg").write_bytes(b"b" * 7)
        candidate = CleanupCandidate(8, 88, "succeeded", 3, str(job_dir), 12, 9999.0)
        result = manager.cleanup_candidate(candidate)
        self.assertTrue(result.complete)
        self.assertEqual(result.freed_bytes, 12)
        self.assertFalse(job_dir.exists())

        job_dir.mkdir()
        (job_dir / "active.mp4.part").write_bytes(b"partial")
        blocked = manager.cleanup_candidate(
            CleanupCandidate(8, 88, "succeeded", 4, str(job_dir), 7, 9999.0)
        )
        self.assertFalse(blocked.complete)
        self.assertIn("partial-file", blocked.errors)
...
class CleanupDryRunIntegrationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_to_waterline_deletes_and_commits_durable_state(self) -> None:
        tempdir = tempfile.TemporaryDirectory()
        self.addCleanup(tempdir.cleanup)
        base = Path(tempdir.name)
        root = base / "downloads"
        root.mkdir()
        repo = SQLiteRepository(base / "state.sqlite3", download_root=root)
        await repo.open()
        await repo.migrate()
        ...
        local_path.write_bytes(b"cache-data")
        ...
        pipeline = SimpleNamespace(
            ...
            disk=DiskManager(root),
            retryable={},
            webdav_keep_cache=set(),
            _disk_cache_retention_hours=0.0,
            _disk_failed_retention_hours=0.0,
        )
        queue = JobQueue(pipeline, shadow=SimpleNamespace(job_ids={120: job.id}))
        report = await queue.cleanup_to_waterline(force=True)
        self.assertEqual(report["cleaned"], 1)
        self.assertEqual(report["failed"], 0)
        self.assertEqual(report["freed_bytes"], 10)
        self.assertFalse(job_dir.exists())
        self.assertEqual(await repo.cleanup_inventory(), [])
        items = await repo.list_job_items(job.id)
        self.assertIsNone(items[0].local_path)
        self.assertEqual((await repo.list_job_events(job.id))[-1].event_type, "disk_cleanup")
