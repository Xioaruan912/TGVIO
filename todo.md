...
- [x] **18+ 模式偏好（/mode）**：每次询问 / 总是雪花 / 总是正常，持久化 `session/prefs.json`；**v12 起默认「总是正常」**——首次使用不再询问/暂存，需要雪花自行 /mode 修改
...
- [x] **合集会话（v11）**：转发自动开始合集（自动 /begin），多次转发汇总为一个 collection 任务——图片（≤10 张，超出按序丢弃）做封面相册、**全部视频进同一个评论区**；每次转发后状态消息弹「🛑 结束并发布」按钮（`SESSION_END_TIMEOUT` 默认 5s 超时隐藏继续等待转发）；`/end`（/结束）或按钮结束，整个合集只问一次 18+；`/begin`（/开始）显式开始
- [x] **v12 清理**：删除 `/pack`（含命令菜单/文案/`_finalize_album` 的 wait 参数）与「未设置模式先暂存」机制（`hold_item`/`_HeldItem`/`HELD_TIMEOUT`/`_release_held`/`_has_pref` 全删）；默认 18+ 模式改为「总是正常」，首次使用不询问
- [x] **常驻按钮（v12.1）**：打字框上方常驻回复键盘「📥 开始合集 / 🛑 结束合集」（`ReplyKeyboardMarkup` persistent），点击文本匹配路由到 begin/end 逻辑
- [x] **会话评论（v11.1）**：会话期间文字消息按发送顺序收集（`_session_add_text`），每次内容按行拆分自动补换行，`/end` 时整合为一条 ≤1024 字符文本作为**封面 caption 与封面一起发到频道**（非评论区）；`/begin` 创建空会话支持先评论后媒体
...
- [x] **已部署到 VPS 并验证**（2026-08-05，`~/deploy_vps.sh`）：容器 Up、`Bot commands registered` + `Bot started` 日志正常、代码 md5 一致
- [ ] 功能实机验证（转发视频/相册/URL）需用户本人操作，Agent 只负责看日志

---

## 2. 历史待办映射（2026-09-01 同步）
...
- [x] **URL 下载进度**：已纳入统一进度/取消路径，旧描述“无进度条/无停止按钮”已过期。
- [ ] **调试日志精简**：`on_private_message` 顶部每消息 INFO 日志正式环境可降 DEBUG
- [ ] **缩略图增强**：截中间帧 / 多帧供选择
- [ ] **多用户支持**：仍属于明确延期的扩展项；当前继续以单用户 `ALLOWED_USERS` 模式运行，不是现阶段 blocker。

> 说明：上面仍未勾选的条目是可选体验/扩展工作或需要用户本人操作的实机验证，不代表当前发布不完整。当前正式 roadmap 没有未勾选 blocker；最新生产发布与验证记录见 `AGENTS.md` 的 2026-09-01 条目。
