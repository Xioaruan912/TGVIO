...
function render() {
  if (loading || !feed.length) {
    ...
    document.querySelector<HTMLDivElement>("#app")!.innerHTML = `<main class="app-shell"><div id="toast" class="toast" role="status"></div><section class="stage"><div class="phone-frame"><div class="safe-head"><div class="mobile-brand"><span class="logo"></span><div><strong>TGVIO Player</strong><small>Private archive feed</small></div></div></div><div class="clip-info"><h1>${loadError || "Loading your archive"}</h1><p>${signIn ? "Enter the Player access secret. It is sent only to this same-origin Player service." : loadError ? "Check the private Player service and try again." : "Preparing a private random feed..."}</p>${signIn ? '<form id="login"><input id="secret" type="password" autocomplete="current-password" aria-label="Player access secret" required><button>Sign in</button></form>' : loadError ? '<button id="retry">Retry</button>' : ""}</div></div></section></main>`;
