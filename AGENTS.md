...
> 最后更新：2026-09-14（R2-18F1～F6 验收缺陷修复已交付：冻结提交恢复、draft 风格、预览资源、结果卡幂等）
>
...
- **2026-09-14 R2-18F 验收缺陷修复增量（权威，覆盖下文旧身份）**：F1～F6 已全部实现、测试并部署。最终生产 full commit `53cbb2d0654b7d1e686216e0e4530d5c88c5ac29`，release `r2-18f6-53cbb2d-20260914T120716Z`，runtime image `sha256:bcb3174a437a23bb1834eb5b14c790fe5f30aa271651e71d5c7ffccf5e352cd6`，Git/宿主/容器 source manifest `fe13b19bc0d54faaff7ed583c8e415047b78dcda2cfa7ed830d9ad307cc706d5`。F6 `53cbb2d`（migration=none）修复冻结后拒收/后台提交恢复/预览硬写入预算/路径fail-closed，证据见 [R2-18F6_REPAIR.md](docs/refactor-v2/R2-18F6_REPAIR.md)。各包：F2 `568acc6`/`0015_frozen_submissions`（v15，冻结提交+崩溃恢复+分块幂等）；F3 `07bc973`/`0016_draft_style`（v16，draft-scoped 风格+同款再发；首次 `fa54582` 因远端并发用例暴露“第二个 token 返回共享 submission 视为成功”而 fail-closed 中止，生产停留 F2）；F4 `4acd2bc`（migration=none，预览字节预算/队列/TTL/清理/生命周期）；F5 `b238b58`（migration=none，收藏幂等+结果卡链接去重）。最终 **475 tests** 全绿；每包独立 postflight healthy、restart=0、单实例、error=0、jobs=0、blockers=[]、quick_check=ok，rollback-check passed。schema v16 / hash `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c` / ledger `1..16`。**仍需真实 Android/iOS 实机验收**；R2-17 精确重复审核仍未实现。证据见 [R2-18F_RELEASE.md](docs/refactor-v2/evidence/R2-18F_RELEASE.md) 与 [R2-18F_ACCEPTANCE_FIXES.md](docs/refactor-v2/R2-18F_ACCEPTANCE_FIXES.md)；下文 R2-18A/D 等身份为上一基线，不覆盖本增量。
