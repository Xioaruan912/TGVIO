...
> 最后更新：2026-09-13（R2-08 Bot UI 热点拆分已交付，R2 系列仅剩 R2-10 收口）
>
...
- R2-01～R2-06 已恢复 Git 权威、可复现发布链、移动端 UX、checksum migration、durable scheduler/intake/control、自动恢复、SQL 任务/失败中心、通用 operation token 与可恢复撤销。R2-07A1 single Archive profile/policy、A2 durable recovery/status UI、A3 exact remote Archive delete、D Diagnostic Snapshot / 静态代理脱敏状态、R2-09 只读 Dashboard/metrics/通知 outbox 与 R2-08 Bot UI 热点拆分已于 2026-09-13 正式发布，R2-07/R2-09 收口且所有 `REQUIRED` 合同为 `VERIFIED`。生产 runtime full commit 为 `52f39cad6c33614daaa742ed2f07dea0c4a6011d`，release 为 `r2-08b-52f39ca-20260913T101837Z`（R2-09 曾以 `0008_notification_outbox` 把 schema 推进到 v8；R2-08/R2-08B 为 migration=none 的行为等价拆分）；退役旧树仍由 annotated tag `legacy-telegram-video-forwarder-750b3c1` 保留。证据见 [`R2-07A1_ARCHIVE_PROFILE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A1_ARCHIVE_PROFILE_RELEASE.md)、[`R2-07A2_ARCHIVE_RECOVERY_RELEASE.md`](docs/refactor-v2/evidence/R2-07A2_ARCHIVE_RECOVERY_RELEASE.md)、[`R2-07A3_ARCHIVE_DELETE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A3_ARCHIVE_DELETE_RELEASE.md)、[`R2-07D_RELEASE.md`](docs/refactor-v2/evidence/R2-07D_RELEASE.md)、[`R2-09_RELEASE.md`](docs/refactor-v2/evidence/R2-09_RELEASE.md) 与 [`R2-08_RELEASE.md`](docs/refactor-v2/evidence/R2-08_RELEASE.md)、[`R2-08B_RELEASE.md`](docs/refactor-v2/evidence/R2-08B_RELEASE.md)。后续 docs-only closure commit 可以领先生产 runtime commit，但不因此构建或重启。
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:507d38f97c3d5d3d60d2d2295ec2c5e850d053554e70cc6af73d9ea6bdf3bd0e`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `9d2b27c3b058c5ee2de70a34e4472eab3c6bdc354fd882bb2c2b3342fe394fb9`。R2-09 已把 schema 推进到 `user_version=8`，schema hash 为 `5b80e9d9aa0abe258883e5eb8b26d54ce2481b4e03daa3ee45f75e477423f76a`；R2-08/R2-08B 未改变 schema。
...
- 正式发布只能从 clean、已推送的 `origin/main` 运行 `scripts/deploy_hostdzire.py`；dirty worktree 只能做无网络诊断构建。R2-07A1/A2/A3/D、R2-09 与 R2-08 已交付，所有 `REQUIRED` 合同 `VERIFIED`；固定单一发布目标，动态 Destination Profiles 与 Proxy Profiles 已由用户明确退役，代理只保留环境变量、启动检测和脱敏状态。R2-04E 大文件性能、R2-05 合集交互和 R2-06 真实撤销仍需受控生产验收；R2-10 自动化收口已交付（旧树退役、release 身份与回滚资产验证），受控生产 smoke 与破坏性回滚演练待 owner 窗口，清单见 [`R2-10_CLOSURE.md`](docs/refactor-v2/evidence/R2-10_CLOSURE.md)。不得绕过 partial/uncertain 外部副作用保护。HostDZire 当前 `ntp_synchronized=no`，应在独立运维窗口修复。
