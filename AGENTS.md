...
> 最后更新：2026-09-14（R2-15 安全历史保留 + 每业务日展示编号已交付）
>
...
- **2026-09-14 R2-18A 增量（覆盖下文 R2-15B runtime 身份）**：首页基础包已发布，full commit `c7ba056e5a6d2451a9556acdf5fb4a2b32eda04f`，release `r2-18a-c7ba056-20260914T025757Z`，image `sha256:73338e97dd2e826ad023cbf20c11aafdde19c480e521bd0fc42316bdbfec7604`，Git/宿主/容器 source manifest `5bde218af46f3ae8ddde713db181ae5327fafedb047f6416c33507296032a9c1`。409 tests；migration=none/schema v11/hash 不变。独立后验 healthy、restart=0、单实例、error=0、jobs=0、blockers=[]、quick_check=ok；rollback-check passed。R2-16/R2-17 仅设计、未实现；R2-18 仅 A 基础包交付，草稿/风格/效果预览等未完成。用户要求交接后停止扩展功能。后续以 [R2-18_HANDOFF.md](docs/refactor-v2/R2-18_HANDOFF.md) 与 [发布证据](docs/refactor-v2/evidence/R2-18A_RELEASE.md) 为准；下文 R2-15B 身份与405 tests为上一基线，不覆盖本增量。
...
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:d4e5ed97353a4dd14c7131ea5b2613783f2f22c246d99a245a97644c5e82b020`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `e22878365bbd0ed6f6c627afefbae1cef5abfd31d255b3a992ab868fba207d78`。R2-15A 已把 schema 推进到 `user_version=10`，R2-15B 推进到 `user_version=11`，schema hash 为 `344e92de1828579e46afe9df40c00ccc0eab10028718ab2d70b8aa7bb896ed6d`。
- 当前生产 SQLite 是 `/root/TGVIO/data/state.sqlite3`：`quick_check=ok`，`jobs=0`（R2-13 每日 06:00 已清理旧记录，R2-15 起不再物理删除），所有业务 blocker 为 0；migration ledger 已连续登记 1～11，`PRAGMA user_version=11`，schema hash 为 `344e92de1828579e46afe9df40c00ccc0eab10028718ab2d70b8aa7bb896ed6d`。`0007_archive_exact_delete`～`0011_display_identity` 均已通过生产副本 rehearsal；部署后 `archive_deletions`/`archive_deletion_targets`/`archive_deletion_events`/`notification_outbox`/`archive_day_counters`/`runtime_flags`/`job_visibility`/`maintenance_runs`/`maintenance_targets`/`job_display_identity` 初始均为 0。任何后续 schema 变化仍必须新增不可变 migration，并先做生产副本演练。
- 当前 R2-15B 正式 clean/pushed test image 在 `--network none` 下 **405 tests 全通过**；测试容器未启动 Bot。runtime image 离线检查继续通过，无 tests、bytecode、构建工具、秘密或运行数据。R2-07D/R2-09 覆盖固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界 TCP 探测（凭据/端点不入结果）、离线 `--check` 不连网的 `configured_unchecked`、只读 Dashboard DTO 脱敏、HTTP 认证/方法/安全头、metrics 低基数，以及 outbox 幂等/claim lease/退避/dead-letter/HMAC；R2-08 覆盖 mixin 拆分与 1000 行源文件预算；R2-11 覆盖 owner 告警脱敏/冷却/主次渠道与合集预览确认前不入队；R2-12/R2-13 覆盖归档慢后端容错、发布竞态修复、短路径布局（每日序号）与每日 06:00 安全维护/运行时开关；R2-14 覆盖 ffprobe 视频属性、缩略图空白帧排除与 1MB 预算、仅终态失败告警、环境能力自检；R2-15 覆盖安全隐藏/历史筛选、durable maintenance run（lease/generation、冻结目标、有界状态卡删除、过期 token/outbox 回收）、`job_display_identity` 分配/回填/业务日解析与按业务日统计。当前生产 Bot、发布、受控 fixture、URL 和 Archive 非敏感开关均为 enabled；owner 告警、合集预览与每日安全维护默认开启；`/diag` 显示 `ffmpeg/ffprobe/yt-dlp/cryptg=true, hachoir=false`；`TGVIO_STATIC_PROXY_URL` 未配置，`/diag` 显示 `disabled`；`TGVIO_DASHBOARD_ENABLED`/`TGVIO_WEBHOOK_ENABLED` 未设置，无 8787 监听端口。
...
- 正式发布只能从 clean、已推送的 `origin/main` 运行 `scripts/deploy_hostdzire.py`；dirty worktree 只能做无网络诊断构建。R2-07A1/A2/A3/D、R2-09、R2-08、R2-11、R2-12、R2-13、R2-14 与 R2-15 已交付，所有 `REQUIRED` 合同 `VERIFIED`；固定单一发布目标，动态 Destination Profiles 与 Proxy Profiles 已由用户明确退役，代理只保留环境变量、启动检测和脱敏状态。R2-04E 大文件性能、R2-05 合集交互和 R2-06 真实撤销仍需受控生产验收；R2-10 自动化收口已交付（旧树退役、release 身份与回滚资产验证），受控生产 smoke 与破坏性回滚演练待 owner 窗口，清单见 [`R2-10_CLOSURE.md`](docs/refactor-v2/evidence/R2-10_CLOSURE.md)。不得绕过 partial/uncertain 外部副作用保护。HostDZire 当前 `ntp_synchronized=no`，应在独立运维窗口修复。
