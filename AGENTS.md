...
- R2-01～R2-06 已恢复 Git 权威、可复现发布链、移动端 UX、checksum migration、durable scheduler/intake/control、自动恢复、SQL 任务/失败中心、通用 operation token 与可恢复撤销。R2-07A1 single Archive profile/policy、A2 durable recovery/status UI、A3 exact remote Archive delete、D Diagnostic Snapshot / 静态代理脱敏状态、R2-09 只读 Dashboard/metrics/通知 outbox 与 R2-08 Bot UI 热点拆分已于 2026-09-13 正式发布，R2-07/R2-09 收口且所有 `REQUIRED` 合同为 `VERIFIED`。生产 runtime full commit 为 `c39fe9d579d86e73c1505741606221408e0b5fd2`，release 为 `r2-08-c39fe9d-20260913T094000Z`（R2-09 曾以 `0008_notification_outbox` 把 schema 推进到 v8；R2-08 为 migration=none 的行为等价拆分）；退役旧树仍由 annotated tag `legacy-telegram-video-forwarder-750b3c1` 保留。证据见 [`R2-07A1_ARCHIVE_PROFILE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A1_ARCHIVE_PROFILE_RELEASE.md)、[`R2-07A2_ARCHIVE_RECOVERY_RELEASE.md`](docs/refactor-v2/evidence/R2-07A2_ARCHIVE_RECOVERY_RELEASE.md)、[`R2-07A3_ARCHIVE_DELETE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A3_ARCHIVE_DELETE_RELEASE.md)、[`R2-07D_RELEASE.md`](docs/refactor-v2/evidence/R2-07D_RELEASE.md)、[`R2-09_RELEASE.md`](docs/refactor-v2/evidence/R2-09_RELEASE.md) 与 [`R2-08_RELEASE.md`](docs/refactor-v2/evidence/R2-08_RELEASE.md)。后续 docs-only closure commit 可以领先生产 runtime commit，但不因此构建或重启。
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:1f931e65a76d5ad3c90cc81ccdd3a07fce10034d7c6ea802b2ba0d494e44111f`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `ba8ad0406e1a32d1a921dd98a0860f860731b006e5fc0f32e37dbb23e94982c0`。R2-09 已把 schema 推进到 `user_version=8`，schema hash 为 `5b80e9d9aa0abe258883e5eb8b26d54ce2481b4e03daa3ee45f75e477423f76a`；R2-08 未改变 schema。
