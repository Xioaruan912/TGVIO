...
> 最后更新：2026-09-14（R2-18F1～F5 验收缺陷修复已交付：冻结提交恢复、draft 风格、预览资源、结果卡幂等）
>
...
- **2026-09-14 R2-18F 验收缺陷修复增量（权威，覆盖下文旧身份）**：F1～F5 已全部实现、测试并部署。最终生产 full commit `b238b5861da6c44c94a737d953f3521027a03fa6`，release `r2-18f5-b238b58-20260914T075349Z`，runtime image `sha256:af9e6a885d1807a8fccf83e3bbb0d8d90a124dfe89813178b3e38489a967c469`，Git/宿主/容器 source manifest `4d6163b928fd641e736f20f02b0d6dcf94599d577767025a68768fd6b182f955`。各包：F2 `568acc6`/`0015_frozen_submissions`（v15，冻结提交+崩溃恢复+分块幂等）；F3 `07bc973`/`0016_draft_style`（v16，draft-scoped 风格+同款再发；首次 `fa54582` 因远端并发用例暴露“第二个 token 返回共享 submission 视为成功”而 fail-closed 中止，生产停留 F2）；F4 `4acd2bc`（migration=none，预览字节预算/队列/TTL/清理/生命周期）；F5 `b238b58`（migration=none，收藏幂等+结果卡链接去重）。最终 **472 tests** 全绿；每包独立 postflight healthy、restart=0、单实例、error=0、jobs=0、blockers=[]、quick_check=ok，rollback-check passed。schema v16 / hash `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c` / ledger `1..16`。**仍需真实 Android/iOS 实机验收**；R2-17 精确重复审核仍未实现。证据见 [R2-18F_RELEASE.md](docs/refactor-v2/evidence/R2-18F_RELEASE.md) 与 [R2-18F_ACCEPTANCE_FIXES.md](docs/refactor-v2/R2-18F_ACCEPTANCE_FIXES.md)；下文 R2-18A/D 等身份为上一基线，不覆盖本增量。
...
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:af9e6a885d1807a8fccf83e3bbb0d8d90a124dfe89813178b3e38489a967c469`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `4d6163b928fd641e736f20f02b0d6dcf94599d577767025a68768fd6b182f955`。R2-15 推进到 v10/v11，R2-16 推进到 v12，R2-18B 推进到 v13，R2-18E 推进到 v14，R2-18F2 推进到 v15，R2-18F3 推进到 v16，schema hash 为 `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c`；R2-18C/R2-18D/R2-18F1/R2-18F4/R2-18F5 为 migration=none。
