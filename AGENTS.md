...
> 最后更新：2026-09-15（R2-18F1～F7 验收缺陷修复已交付：冻结提交恢复、draft 风格、预览资源、结果卡幂等、启动恢复、效果预览入口）
>
...
- **2026-09-15 R2-18F 验收缺陷修复增量（权威，覆盖下文旧身份）**：F1～F7 已全部实现、测试并部署。最终生产 full commit `31a51d26eadbbb7aacc1bbc584bd04aafc1ed6ca`，release `r2-18f7-31a51d2-20260915T025757Z`，runtime image `sha256:52d96b8c2aea5a263754a7fb559fefd03f54665f0d1dfb335732e5acd991e93f`，Git/宿主/容器 source manifest `7759913efd2ef44ca30b8d273e4ff1ddbb9a41209c52eab4646dd100b46d2b23`。各包：F2 `568acc6`/`0015_frozen_submissions`（v15）；F3 `07bc973`/`0016_draft_style`（v16）；F4 `4acd2bc`；F5 `b238b58`；F6 `53cbb2d`（migration=none，冻结后拒收/后台提交恢复/预览硬写入预算/路径 fail-closed，475 tests，见 [R2-18F6_REPAIR.md](docs/refactor-v2/R2-18F6_REPAIR.md)）；F7 `31a51d2`（migration=none，“👀 预览与整理”+首层“🖼 生成效果预览”、新建/结束/预览文本路由 NameError 修复、清理时间动态显示）。最终 **479 tests** 全绿；postflight healthy、restart=0、单实例、error=0、blockers=[]、quick_check=ok，rollback-check passed。schema v16 / hash `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c` / ledger `1..16`。F7 首次构建在切换后、写元数据前被中断（`.release-commit`/`current` 停留 F6，`remote_preflight` 报 release-commit/source-manifest）；已建立在线备份、用发布脚本 `atomic_metadata` 语义对齐到正在运行的 F7，再从 clean `31a51d2` 重跑唯一入口收口。**仍需真实 Android/iOS 实机验收**；R2-17 精确重复审核仍未实现。证据见 [R2-18F7_RELEASE.md](docs/refactor-v2/evidence/R2-18F7_RELEASE.md)、[R2-18F_RELEASE.md](docs/refactor-v2/evidence/R2-18F_RELEASE.md)。
...
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:52d96b8c2aea5a263754a7fb559fefd03f54665f0d1dfb335732e5acd991e93f`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `7759913efd2ef44ca30b8d273e4ff1ddbb9a41209c52eab4646dd100b46d2b23`。R2-15 推进到 v10/v11，R2-16 推进到 v12，R2-18B 推进到 v13，R2-18E 推进到 v14，R2-18F2 推进到 v15，R2-18F3 推进到 v16，schema hash 为 `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c`；R2-18C/D/F1/F4/F5/F6/F7 为 migration=none。
- 当前生产 SQLite 是 `/root/TGVIO/data/state.sqlite3`：`quick_check=ok`，`jobs_total=3`（均为终态历史任务；R2-15 起不再物理删除），所有业务 blocker 为 0（非终态 Job/claim/Archive 均为 0）；migration ledger 已连续登记 1～16，`PRAGMA user_version=16`，schema hash 为 `59624f44635dd5ff7a31f313780d0aa4669cbf4bf18efae7405f98ec4ea0102c`。`0012_collection_editing`～`0016_draft_style` 均已通过生产副本 rehearsal；新表 `collection_drafts`/`collection_entry_edits`/`collection_submissions`/`editing_interactions`/`favorites`/`suggestion_applications`/`preview_requests`/`collection_part_jobs` 初始均为 0。任何后续 schema 变化仍必须新增不可变 migration，并先做生产副本演练。
- 当前 R2-18F7 正式 clean/pushed test image 在 `--network none` 下 **479 tests 全通过**；测试容器未启动 Bot。runtime image 离线检查继续通过，无 tests、bytecode、构建工具、秘密或运行数据。R2-07D/R2-09 覆盖固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界 TCP 探测（凭据/端点不入结果）、离线 `--check` 不连网的 `configured_unchecked`、只读 Dashboard DTO 脱敏、HTTP 认证/方法/安全头、metrics 低基数，以及 outbox 幂等/claim lease/退避/dead-letter/HMAC；R2-08 覆盖 mixin 拆分与 1000 行源文件预算；R2-11 覆盖 owner 告警脱敏/冷却/主次渠道与合集预览确认前不入队；R2-12/R2-13 覆盖归档慢后端容错、发布竞态修复、短路径布局（每日序号）与每日 06:00 安全维护/运行时开关；R2-14 覆盖 ffprobe 视频属性、缩略图空白帧排除与 1MB 预算、仅终态失败告警、环境能力自检；R2-15 覆盖安全隐藏/历史筛选、durable maintenance run（lease/generation、冻结目标、有界状态卡删除、过期 token/outbox 回收）、`job_display_identity` 分配/回填/业务日解析与按业务日统计；R2-16 覆盖草稿 overlay/revision CAS/冻结提交幂等与旧入口兼容；R2-18 覆盖结果卡/链接策略/收藏/安静模式、风格冻结、整理建议撤回与有界效果预览；R2-18F 覆盖 T1 冻结快照原子落库/崩溃恢复/分块幂等/双 token 双连接竞争、draft-scoped 风格优先级与同款再发、预览字节预算/队列 TTL/路径安全/生命周期回收、收藏目标状态幂等与撤销链接去重、F6 冻结后拒收/启动提交恢复/流式硬预算/符号链接拒绝、F7 预览入口与文本路由。当前生产 Bot、发布、受控 fixture、URL 和 Archive 非敏感开关均为 enabled；owner 告警、合集预览与每日安全维护默认开启；`daily_cleanup_time=19:00`（Asia/Shanghai，业务日编号边界仍 06:00）；`ntp_synchronized=yes`；`/diag` 显示 `ffmpeg/ffprobe/yt-dlp/cryptg=true, hachoir=false`；`TGVIO_STATIC_PROXY_URL` 未配置，`/diag` 显示 `disabled`；`TGVIO_DASHBOARD_ENABLED`/`TGVIO_WEBHOOK_ENABLED` 未设置，无 8787 监听端口。
