...
> 最后更新：2026-09-13（R2-11 owner 告警 + 合集预览已交付）
>
...
- R2-01～R2-06 已恢复 Git 权威、可复现发布链、移动端 UX、checksum migration、durable scheduler/intake/control、自动恢复、SQL 任务/失败中心、通用 operation token 与可恢复撤销。R2-07A1 single Archive profile/policy、A2 durable recovery/status UI、A3 exact remote Archive delete、D Diagnostic Snapshot / 静态代理脱敏状态、R2-09 只读 Dashboard/metrics/通知 outbox、R2-08 Bot UI 热点拆分与 R2-11 owner 告警/合集预览已于 2026-09-13 正式发布，R2-07/R2-09 收口且所有 `REQUIRED` 合同为 `VERIFIED`。生产 runtime full commit 为 `7101d2a5c660ddd64cf08f41c68f8be650a6e291`，release 为 `r2-11-7101d2a-20260913T122158Z`（R2-09 曾以 `0008_notification_outbox` 把 schema 推进到 v8；R2-08/R2-11 为 migration=none 的行为等价/产品增强发布）；退役旧树仍由 annotated tag `legacy-telegram-video-forwarder-750b3c1` 保留。证据见 [`R2-07A1_ARCHIVE_PROFILE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A1_ARCHIVE_PROFILE_RELEASE.md)、[`R2-07A2_ARCHIVE_RECOVERY_RELEASE.md`](docs/refactor-v2/evidence/R2-07A2_ARCHIVE_RECOVERY_RELEASE.md)、[`R2-07A3_ARCHIVE_DELETE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A3_ARCHIVE_DELETE_RELEASE.md)、[`R2-07D_RELEASE.md`](docs/refactor-v2/evidence/R2-07D_RELEASE.md)、[`R2-09_RELEASE.md`](docs/refactor-v2/evidence/R2-09_RELEASE.md) 与 [`R2-08_RELEASE.md`](docs/refactor-v2/evidence/R2-08_RELEASE.md)、[`R2-08B_RELEASE.md`](docs/refactor-v2/evidence/R2-08B_RELEASE.md) 与 [`R2-11_RELEASE.md`](docs/refactor-v2/evidence/R2-11_RELEASE.md)。后续 docs-only closure commit 可以领先生产 runtime commit，但不因此构建或重启。
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:c78f3eb4992192281c79a7243eb4926573acb4436cdc72fa4dddd126cbe63d79`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `932cad1e7df3a2a76098f214d73188ed6ecfa4323131c5f5d52951dc44c44c8f`。R2-09 已把 schema 推进到 `user_version=8`，schema hash 为 `5b80e9d9aa0abe258883e5eb8b26d54ce2481b4e03daa3ee45f75e477423f76a`；R2-08/R2-11 未改变 schema。
...
- 当前 R2-11 正式 clean/pushed test image 在 `--network none` 下 **380 tests 全通过**；测试容器未启动 Bot。runtime image 离线检查继续通过，无 tests、bytecode、构建工具、秘密或运行数据。R2-07D/R2-09 覆盖固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界 TCP 探测（凭据/端点不入结果）、离线 `--check` 不连网的 `configured_unchecked`、只读 Dashboard DTO 脱敏、HTTP 认证/方法/安全头、metrics 低基数，以及 outbox 幂等/claim lease/退避/dead-letter/HMAC；R2-08 覆盖 mixin 拆分与 1000 行源文件预算；R2-11 覆盖 owner 告警脱敏/冷却/主次渠道与合集预览确认前不入队。当前生产 Bot、发布、受控 fixture、URL 和 Archive 非敏感开关均为 enabled；owner 告警与合集预览默认开启；`TGVIO_STATIC_PROXY_URL` 未配置，`/diag` 显示 `disabled`；`TGVIO_DASHBOARD_ENABLED`/`TGVIO_WEBHOOK_ENABLED` 未设置，无 8787 监听端口。
...
- 正式发布只能从 clean、已推送的 `origin/main` 运行 `scripts/deploy_hostdzire.py`；dirty worktree 只能做无网络诊断构建。R2-07A1/A2/A3/D、R2-09、R2-08 与 R2-11 已交付，所有 `REQUIRED` 合同 `VERIFIED`；固定单一发布目标，动态 Destination Profiles 与 Proxy Profiles 已由用户明确退役，代理只保留环境变量、启动检测和脱敏状态。R2-04E 大文件性能、R2-05 合集交互和 R2-06 真实撤销仍需受控生产验收；R2-10 自动化收口已交付（旧树退役、release 身份与回滚资产验证），受控生产 smoke 与破坏性回滚演练待 owner 窗口，清单见 [`R2-10_CLOSURE.md`](docs/refactor-v2/evidence/R2-10_CLOSURE.md)。不得绕过 partial/uncertain 外部副作用保护。HostDZire 当前 `ntp_synchronized=no`，应在独立运维窗口修复。
