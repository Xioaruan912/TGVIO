...
> 最后更新：2026-09-13（R2-13 短路径归档 + 每日 06:00 清空 + Bot 内开关已交付）
>
...
- R2-01～R2-06 已恢复 Git 权威、可复现发布链、移动端 UX、checksum migration、durable scheduler/intake/control、自动恢复、SQL 任务/失败中心、通用 operation token 与可恢复撤销。R2-07A1 single Archive profile/policy、A2 durable recovery/status UI、A3 exact remote Archive delete、D Diagnostic Snapshot / 静态代理脱敏状态、R2-09 只读 Dashboard/metrics/通知 outbox、R2-08 Bot UI 热点拆分、R2-11 owner 告警/合集预览、R2-12 归档慢后端容错与 R2-13 短路径归档/每日清空/运行时开关已于 2026-09-13 正式发布，R2-07/R2-09 收口且所有 `REQUIRED` 合同为 `VERIFIED`。生产 runtime full commit 为 `9f0eb946c2aa036cc203816ea5484e0bc1a884a3`，release 为 `r2-13-9f0eb94-20260913T135837Z`（R2-09 以 `0008_notification_outbox` 推进到 v8，R2-13 以 `0009_archive_layout_flags` 推进到 v9；R2-12 为 migration=none）；退役旧树仍由 annotated tag `legacy-telegram-video-forwarder-750b3c1` 保留。证据见 [`R2-07A1_ARCHIVE_PROFILE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A1_ARCHIVE_PROFILE_RELEASE.md)、[`R2-07A2_ARCHIVE_RECOVERY_RELEASE.md`](docs/refactor-v2/evidence/R2-07A2_ARCHIVE_RECOVERY_RELEASE.md)、[`R2-07A3_ARCHIVE_DELETE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A3_ARCHIVE_DELETE_RELEASE.md)、[`R2-07D_RELEASE.md`](docs/refactor-v2/evidence/R2-07D_RELEASE.md)、[`R2-09_RELEASE.md`](docs/refactor-v2/evidence/R2-09_RELEASE.md)、[`R2-08_RELEASE.md`](docs/refactor-v2/evidence/R2-08_RELEASE.md)、[`R2-08B_RELEASE.md`](docs/refactor-v2/evidence/R2-08B_RELEASE.md)、[`R2-11_RELEASE.md`](docs/refactor-v2/evidence/R2-11_RELEASE.md)、[`R2-12_RELEASE.md`](docs/refactor-v2/evidence/R2-12_RELEASE.md) 与 [`R2-13_RELEASE.md`](docs/refactor-v2/evidence/R2-13_RELEASE.md)。后续 docs-only closure commit 可以领先生产 runtime commit，但不因此构建或重启。
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:5442ffa8c2f6858a9a245c0e21c042e71cc88cb336d6d9bcc1408a2c76458d58`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `daf84fc92682494fdec13243508fce614e95c37fe884b598a7ed2cec5e3b3e8d`。R2-13 已把 schema 推进到 `user_version=9`，schema hash 为 `bb9c405364ac74a758167d5b324149c0543be6812cf0e7fac3c190e52fe17a0e`。
...
- 当前 R2-13 正式 clean/pushed test image 在 `--network none` 下 **388 tests 全通过**；测试容器未启动 Bot。runtime image 离线检查继续通过，无 tests、bytecode、构建工具、秘密或运行数据。R2-07D/R2-09 覆盖固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界 TCP 探测（凭据/端点不入结果）、离线 `--check` 不连网的 `configured_unchecked`、只读 Dashboard DTO 脱敏、HTTP 认证/方法/安全头、metrics 低基数，以及 outbox 幂等/claim lease/退避/dead-letter/HMAC；R2-08 覆盖 mixin 拆分与 1000 行源文件预算；R2-11 覆盖 owner 告警脱敏/冷却/主次渠道与合集预览确认前不入队；R2-12/R2-13 覆盖归档慢后端容错、发布竞态修复、短路径布局（每日序号）与每日 06:00 清空/复位编号/日志保留/运行时开关。当前生产 Bot、发布、受控 fixture、URL 和 Archive 非敏感开关均为 enabled；owner 告警、合集预览与每日清空默认开启；`TGVIO_STATIC_PROXY_URL` 未配置，`/diag` 显示 `disabled`；`TGVIO_DASHBOARD_ENABLED`/`TGVIO_WEBHOOK_ENABLED` 未设置，无 8787 监听端口。
