...
> 最后更新：2026-09-13（R2-07D Diagnostic Snapshot 已交付，R2-07 收口）
>
...
- R2-01～R2-06 已恢复 Git 权威、可复现发布链、移动端 UX、checksum migration、durable scheduler/intake/control、自动恢复、SQL 任务/失败中心、通用 operation token 与可恢复撤销。R2-07A1 single Archive profile/policy、A2 durable recovery/status UI、A3 exact remote Archive delete 与 D Diagnostic Snapshot / 静态代理脱敏状态已于 2026-09-13 正式发布，R2-07 收口。生产 runtime full commit 为 `10b6dd5f58f8839a6fa51428a32b93d8f95c098c`，release 为 `r2-07d-10b6dd5-20260913T091106Z`（migration=none）；退役旧树仍由 annotated tag `legacy-telegram-video-forwarder-750b3c1` 保留。证据见 [`R2-07A1_ARCHIVE_PROFILE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A1_ARCHIVE_PROFILE_RELEASE.md)、[`R2-07A2_ARCHIVE_RECOVERY_RELEASE.md`](docs/refactor-v2/evidence/R2-07A2_ARCHIVE_RECOVERY_RELEASE.md)、[`R2-07A3_ARCHIVE_DELETE_RELEASE.md`](docs/refactor-v2/evidence/R2-07A3_ARCHIVE_DELETE_RELEASE.md) 与 [`R2-07D_RELEASE.md`](docs/refactor-v2/evidence/R2-07D_RELEASE.md)。后续 docs-only closure commit 可以领先生产 runtime commit，但不因此构建或重启。
- HostDZire shared root 为 `/root/TGVIO`，current symlink 为 `/root/TGVIO-current`，Compose service/容器为 `tgvio`，包名为 `src/tgvio`。生产 `.release-commit` 与容器 `APP_COMMIT` 均为上述 full commit，runtime image ID 为 `sha256:0b88a34b1055015f2397b164c0ee9885c1cf5fe3cf9552c501f54e4c35051333`；容器 `running`、health=`healthy`、restart=0、单实例。Git/宿主/容器规范化 Python 源码 manifest 均为 `5d29d0b9d6a685eb8b3e98fbbc1b62be574ac55b01f9412e28f77f8f892c4333`。R2-07D 未改变 schema，生产仍为 `user_version=7`，schema hash 保持 `9cf2d4008d4fb888f5affdffea1ccd413b51968d14d234e07d0769cfe60c6e90`。
- 当前生产 SQLite 是 `/root/TGVIO/data/state.sqlite3`：`quick_check=ok`，24 个 Job（17 succeeded、4 cancelled、3 failed），所有业务 blocker 为 0；migration ledger 已连续登记 1～7，`PRAGMA user_version=7`，schema hash 为 `9cf2d4008d4fb888f5affdffea1ccd413b51968d14d234e07d0769cfe60c6e90`。`0007_archive_exact_delete` 已通过生产 v6 副本 rehearsal；部署后 `archive_deletions`、`archive_deletion_targets`、`archive_deletion_events` 三表初始均为 0。任何后续 schema 变化仍必须新增不可变 migration，并先做生产副本演练。
- 当前 R2-07D 正式 clean/pushed test image 在 `--network none` 下 **350 tests 全通过**；测试容器未启动 Bot。runtime image 离线检查继续通过，无 tests、bytecode、构建工具、秘密或运行数据。R2-07D 覆盖固定 DiagnosticSnapshot 白名单 DTO、3 条有界 SQL 聚合、1000+ Job 不加载历史、release/schema/lease/scheduler/Archive/feature 脱敏投影、静态代理有界 TCP 探测（凭据/端点不入结果）、离线 `--check` 不连网的 `configured_unchecked`，以及 secret/path fixture 拒绝。当前生产 Bot、发布、受控 fixture、URL 和 Archive 非敏感开关均为 enabled；`TGVIO_STATIC_PROXY_URL` 未配置，`/diag` 显示 `disabled`。
...
- 正式发布只能从 clean、已推送的 `origin/main` 运行 `scripts/deploy_hostdzire.py`；dirty worktree 只能做无网络诊断构建。R2-07A1/A2/A3/D 已交付，R2-07 收口；固定单一发布目标，动态 Destination Profiles 与 Proxy Profiles 已由用户明确退役，代理只保留环境变量、启动检测和脱敏状态。R2-04E 大文件性能、R2-05 合集交互和 R2-06 真实撤销仍需受控生产验收。不得绕过 partial/uncertain 外部副作用保护。HostDZire 当前 `ntp_synchronized=no`，应在独立运维窗口修复。
