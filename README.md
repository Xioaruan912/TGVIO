...
- **自动来源**：`/sources` 可把 Bot 已加入的频道/群组新媒体自动送入现有下载、发布、去重与 WebDAV 流程；原生相册按 2 秒短窗口收齐，来源逐条发布的连续视频按“最后一条后静默 120 秒”组成一个媒体组，满 10 个立即提交
- **来源重启恢复**：来源 update 到达后先写入 SQLite；120 秒聚合窗口内重启只按已接收的精确消息 ID 恢复，不扫描或补发来源历史。文本、图片、非视频文档和新的原生媒体组会结束前一批连续视频
...
若使用 `/sources` 自动中转，还必须把 Bot 加入每个来源频道；广播频道应设为管理员，否则 Telegram 不会把该频道的新消息 update 交给 Bot。
...
- `/sources` → 管理自动来源 Profile；创建、检查和重新启用时验证 Bot 自身来源成员身份。只处理 Bot 加入后收到的新媒体，不补历史；逐条连续视频静默 120 秒后合并，满 10 个立即提交
...
- **自动来源已启用但没有任务**：先在 `/sources` 点“检查访问”，并确认 Bot 已作为来源频道管理员加入。旧消息不会回扫；修复权限后请发送一条新媒体验证。
