...
7. **从"不能转发"的来源推送内容**（可选，需要个人账号登录一次）：
   - 场景：某些资源机器人/频道禁止转发或下载，普通转发给机器人没用。
   - 用法一：在那个聊天里**回复你想保存的那条**，然后发送 **`#tgvio`**；抓取成功后触发消息会自动删除。
   - 用法二：把频道的消息链接（`https://t.me/…`）直接发给机器人。
   - 只在你配置的**来源白名单**里生效，不会自动监听任何来源。
   - 登录：`docker exec -it tgvio python /app/scripts/login_source_session.py`，按提示输入手机号/验证码/两步密码。
