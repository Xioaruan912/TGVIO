...
7. **从"不能转发"的来源推送内容**（可选，全程在手机 Bot 里操作）：
   ...
   - 抓取方式一：在那个聊天里**回复你想保存的那条**，发送 **`#tgvio`**（抓取后触发消息自动删除）。
   - 抓取方式二：直接发送 **`#tgvio`** —— 默认会抓该聊天**最近一条媒体**（可在 `/source` 页面用「🎯 无回复模式」开/关）。
   - 抓取方式三：把频道的消息链接（`https://t.me/…`）发给机器人。
   - 抓取方式四（最稳）：在 TGVIO 聊天里发送 **`/grab`**，直接抓「第 1 个来源」最近一条媒体；有多个来源时用 **`/grab 2`** 指定第 2 个。
