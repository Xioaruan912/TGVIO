...
7. **从"不能转发"的来源推送内容**（可选，全程在手机 Bot 里操作）：
   ...
   - 抓取方式一（推荐）：发送 **`/pick`**，机器人列出该来源**最近 10 条媒体**（相册合并为一条），每行只给数量与大小，例如 `3) 相册 10 项 · 🎬7 🖼3 · 175.1MB`：
     - 打开页面后会**自动生成一张整页缩略图**（位置对应行号），生成期间有 `⏳ 正在生成… 3/10` 进度提示；只取每条的**内嵌缩略图**（几十 KB）；
     - 图片若没有内嵌缩略图会自动取原图（≤2MB），视频若没有缩略图只拉前 4MB 抽一帧（≤6 秒），仍然失败才显示"无预览"；
     - 每行两个按钮：**📥 = 抓取**（先弹确认卡，✅ 才真正入队）、**👁 = 只看缩略图**（60 秒后自动删除）；
     - 「只看视频」可过滤掉纯图片广告；
