...
7. **从"不能转发"的来源推送内容**（可选，全程在手机 Bot 里操作）：
   ...
   - 抓取方式一（推荐）：发送 **`/pick`**，机器人列出该来源**最近 10 条媒体**（相册合并为一条，图片+视频相册会整组），每行带 🖼/🎬 构成、大小、时间和 `#源消息号`：
     - 打开页面后会**自动生成一张整页缩略图**（5×2 网格，位置对应行号），生成期间有 `⏳ 正在生成… 3/10` 进度提示；
     - 每行两个按钮：**📥 = 抓取**（先弹确认卡，显示数量/大小，✅ 才真正入队）、**👁 = 只看缩略图**（60 秒后自动删除）；
     - 「🎬 只看视频」可过滤掉纯图片广告；
